		...
		ISidedInventory
{
	...
	public ItemStack decrStackSize(int i, int j)
	{
		if (this.furnaceContents[i] != null)
		{
			...
			{
				itemstack = this.furnaceContents[i];
				this.furnaceContents[i] = null;
				return itemstack;
			}
			else
			{
				itemstack = this.furnaceContents[i].splitStack(j);
	...
	public void readFromNBT(NBTTagCompound nbtTagCompound)
	{
		...
		this.currentItemBurnTime = getItemBurnTime(this.furnaceContents[type
				.getFirstFuelSlot()]);
	...
	public void updateEntity()
	{
		...
		if (!this.worldObj.isRemote)
		{
			...
			{
				this.currentItemBurnTime = this.furnaceBurnTime = getItemBurnTime(this.furnaceContents[type
						.getFirstFuelSlot()]);
				if (this.isBurning())
				{
					inventoryChanged = true;
					if (this.furnaceContents[type.getFirstFuelSlot()] != null)
					{
						...
						{
							this.furnaceContents[type.getFirstFuelSlot()] = this.furnaceContents[type
	...
	private boolean canSmelt(int id)
	{
		...
		else
		{
			ItemStack var1 = FurnaceRecipes.smelting().getSmeltingResult(
					this.furnaceContents[type.getFirstInputSlot(id)]);
			if (var1 == null)
				return false;
			if (this.furnaceContents[type.getFirstOutputSlot(id)] == null)
				return true;
			if (!this.furnaceContents[type.getFirstOutputSlot(id)]
					.isItemEqual(var1))
				return false;
	...
	public void smeltItem(int id)
	{
		if (this.canSmelt(id))
		{
			...
			else if (this.furnaceContents[type.getFirstOutputSlot(id)]
					.isItemEqual(var1))
			{
